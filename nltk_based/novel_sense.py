...
def novelwords(text):
    '''Returns sorted set of words according to their "sense score". Based on Wordnet corpus.'''
    def meansimilarity(word):
        #Helper function. Calculate mean path similarity of first synset of the word with all synsets of this word.
        sum = 0.0
    ...
    for token in range(0, len(filtered)):
        ...
        for synset in wn.synsets(mainstring):
            if prevstring != "" and nextstring != "":
                result[mainstring] = [meansimilarity(mainstring), meansimilarity(prevstring), meansimilarity(nextstring)]
            elif prevstring == "" and nextstring != "":        
                result[mainstring] = [meansimilarity(mainstring), meansimilarity(nextstring)]
            elif prevstring != "" and nextstring == "":
                result[mainstring] = [meansimilarity(mainstring), meansimilarity(prevstring)]
